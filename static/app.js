                        ...
                        <tr>
                            ...
                            <td>
                                <span class="service-dns-count" 
                                      ...
                                      onmouseenter="showServiceDnsTooltip(this, '${escapeHtml(appName)}', '${escapeHtml(appNamespace)}')"
...
function showServiceDnsTooltip(element, appName, appNamespace) {
    ...
    serviceDnsTooltipTimeout = setTimeout(() => {
        ...
        // Add full service DNS
        tooltipContent += `<div class="tooltip-section-title" style="margin-bottom: 8px; padding-bottom: 4px; border-bottom: 1px solid #e9ecef;">Service DNS</div>`;
        tooltipContent += `<div class="tooltip-item">
            <span class="tooltip-status" style="background-color: #3b82f6;"></span>
            <span class="tooltip-item-value">${escapeHtml(fullDns)}</span>
        </div>`;
        
        // Add pod DNS entries if available
        const cacheKey = `${appNamespace}/${appName}`;
        const data = replicaCache.get(cacheKey);
        
        if (data && data.pods && data.pods.length > 0) {
            tooltipContent += `<div style="border-bottom: 1px solid #e9ecef; margin: 8px 0;"></div>`;
            tooltipContent += `<div class="tooltip-section-title" style="margin: 8px 0 8px 0; padding-bottom: 4px; border-bottom: 1px solid #e9ecef;">Pod DNS (${data.pods.length})</div>`;
            
            data.pods.forEach((pod, podIndex) => {
                const podDns = `${escapeHtml(pod.name)}.${escapeHtml(appName)}.${escapeHtml(appNamespace)}.svc.cluster.local`;
                const statusColor = pod.phase === 'Running' ? '#2ecc71' : '#f39c12';
                
                tooltipContent += `<div class="tooltip-item">
                    <span class="tooltip-status" style="background-color: ${statusColor};"></span>
                    <span class="tooltip-item-value" style="font-size: 12px;">${podDns}</span>
                </div>`;
                
                // Add separator between pods (but not after the last one)
                if (podIndex < data.pods.length - 1) {
                    tooltipContent += `<div style="border-bottom: 1px solid #f0f0f0; margin: 4px 0;"></div>`;
                }
            });
        }
...
function showProtectionPlanAppsTooltip(element) {
    // Get applications from data attribute (URL-encoded)
    let applicationsJson = element.getAttribute('data-applications') || '%5B%5D';
    
    // Decode URL-encoded JSON
    try {
        applicationsJson = decodeURIComponent(applicationsJson);
    } catch (e) {
        console.error('Failed to decode applications data:', e);
        return;
    }
    ...
    try {
        ...
        console.error('Failed to parse applications JSON:', e, 'Raw:', applicationsJson);
        return;
    }
    ...
    // Add delay before showing tooltip to allow text selection
    protectionPlanAppsTooltipTimeout = setTimeout(() => {
        ...
        applications.forEach((app, appIndex) => {
            ...
            const appName = typeof app === 'string' ? app : (app.name || 'Unknown');
            const appNamespace = typeof app === 'string' ? '' : (app.namespace || '');
            ...
            tooltipContent += `<div class="tooltip-item">
                ...
                <span class="tooltip-item-value">${escapeHtml(appName)}${appNamespace ? ` <span style="color: #6c757d;">(${escapeHtml(appNamespace)})</span>` : ''}</span>
            </div>`;
        ...
        // Adjust if tooltip goes off screen on right
        if (left + tooltipRect.width > window.innerWidth) {
            left = window.innerWidth - tooltipRect.width - 10;
        }
        
        // Adjust if tooltip goes off screen on left
        if (left < 5) {
            left = 5;
        }
            ...
            <table>
                ...
                <tbody>
                    ${data.map(plan => {
                        ...
                        const applicationsJson = encodeURIComponent(JSON.stringify(plan.applications || []));
                        const appCount = (plan.applications || []).length;
                        
                        if (selectionMode === 'by-label' && plan.labelSelectorKey && plan.labelSelectorValue) {
                            const labelDisplay = `${escapeHtml(plan.labelSelectorKey)}=${escapeHtml(plan.labelSelectorValue)}`;
                            selectionDisplay = `<div 
                                class="selection-badge"
                                onmouseenter="showProtectionPlanAppsTooltip(this)"
                                onmouseleave="scheduleHideProtectionPlanAppsTooltip()"
                                data-applications="${applicationsJson}"
                                title="Click to see protected applications">
                                <span class="selection-badge-text">
                                    <span class="selection-badge-label">${labelDisplay}</span>
                                    <span class="selection-badge-count">${appCount}</span>
                                </span>
                            </div>`;
                            ...
                            selectionDisplay = `<div 
                                ...
                                title="Click to see protected applications">
                                <span class="selection-badge-text">
                                    <span class="selection-badge-label">By Name</span>
                                    <span class="selection-badge-count">${appCount}</span>
                                </span>
                            </div>`;
                        }
...
async function showCreatePlanModal() {
    ...
    labelKeySelect.innerHTML = '<option value="">-- Select a label key --</option><option value="__custom__" style="font-style: italic; color: #667eea;">Enter custom key...</option>';
...
function populateLabelKeyOptions() {
    ...
    if (!selectedNamespace || !allData.applications) {
        labelKeySelect.innerHTML += '<option value="__custom__" style="font-style: italic; color: #667eea;">Enter custom key...</option>';
...
function updateLabelValueOptions() {
    ...
    if (!selectedNamespace || !allData.applications) {
        labelValueSelect.innerHTML += '<option value="__custom__" style="font-style: italic; color: #667eea;">Enter custom value...</option>';
...
metadata:
  ...
  annotations:
    ...
    ndk-dashboard/retention-duration: "{{RETENTION_DURATION}}"
spec:
